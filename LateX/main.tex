...
\usepackage[letterpaper,top=7mm,bottom=7mm,left=5mm,right=5mm,marginparwidth=1.75cm]{geometry}
...
\usepackage{multirow}
\usepackage{float}
\usepackage[skip=2pt, indent=40pt]{parskip}
...
\def\nameP{Guilherme Fernandes}
\def\istidP{ist1103847}
\def\nameDU{Duarte Alexandrino}
\def\istidDU{ist1103169}
\def\nameE{Eduardo Silva}
\def\istidE{ist1103240}
\def\nameDA{João Matias}
\def\istidDA{ist1103572}
\def\nameJ{Joaquim Pereira}
\def\istidJ{ist1103871}
...
\begin{titlepage}
    ...
    % Logotipo
    \vspace*{1cm}
    \includegraphics[width=10cm]{Imagens/transferir.jpg}

    % Espaço entre o logotipo e o título
    \vspace{5cm}

    % Título
    {\Huge \textbf{Deep Learning Optimization for High Power Lasers Aberrations Correction}} \\
    \vspace{5cm}

    % Subtítulo ou descrição
    {\Large Experimental Physics Laboratory in Research Units} \\
    \vspace{1.5cm}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.26\textwidth}
    \includegraphics[width=\linewidth]{Imagens/hhg_vs_oa.png}
    \caption{HHG´s Total Signal vs Oblique Astigmatism}
    \label{fig:grafico_hhg_oa}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.21\textwidth}
    \includegraphics[width=\linewidth]{Imagens/240111_PM035132MS781_ob_ast_0.18.jpg}
    \caption{Map of Intensities (0.18)}
    \label{fig:imagem2}
  \end{subfigure}
  \label{fig:imagens}
\end{figure}

As shown in \ref{fig:grafico_hhg_oa}, it is possible to manually find the coefficients that have the greatest effect on the total signal. However, when dealing with higher dimensionality problems (there are many more parameters to optimize besides oblique astigmatism), this method is not optimal. Thus, to make this process faster, it is important to consider Machine Learning approaches, some of which will be detailed in sections \ref{sec4} and \ref{sec6}.
...
\begin{multicols}{2}
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.18\textwidth]{Imagens/primeira.png}
      \caption{Harmonics Signal using ALEX-i Camera.}
      \label{fig:fourier1}
    \end{figure}
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.18\textwidth]{Imagens/fourier.png}
...
Therefore each value of $\lambda_{i}$ will create fringes with separation $w_{i}$. However, it is hard to extract from the image above what are these separations, thus we do Fourier analysis (2D spatial Fourier transform), to get the result in \ref{fig:fourier2}.



When we apply the Fourier transform, we clearly observe peaks corresponding to the spatial frequency of $k_{i} = 2 * \frac{\pi}{w_{i}}$. This way, it is possible to measure the intensity of each peak inside the red box on the image above and get to know the distance between the peaks. To get a graph that enables the measuring of $k_{i}$, we used another algorithm to look at the intensity value of every pixel along a line (that best suits the interference pattern on the 2D Fourier Transform), such as in \ref{fig:fourier3}, giving us the following plot:

\begin{figure}[H]
  \centering
  \includegraphics[width=0.20\textwidth]{Imagens/reta.jpg}
  \caption{Second Algorithm used in the Fourier Space.}
  \label{fig:fourier3}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\textwidth]{Imagens/peaks.png}
  \caption{Intensity of Pixels Along the Polynomial Curve in \ref{fig:fourier3}.}
  \label{fig:fourier4}
\end{figure}

Using the graph above, we can measure directly the values of $k_{i}$, and know the different wavelengths that produced the figure \ref{fig:fourier1}. To get the value of $k_{i}$ we look at peaks pixel indexes and using the ALEX-i camera specifications (knowing that the distance of each pixel is 13.5 $\mu m$), it is possible to get the correspondent value of the spatial frequency. Using the dataset we had, we got the following table:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.25\textwidth]{Imagens/NN_Train_Img1.jpg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\textwidth]{Imagens/setup1.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\textwidth]{Imagens/Zernike.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.35\textwidth]{Imagens/transferir.png}
